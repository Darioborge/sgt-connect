...
BEGIN
  ...
  IF _existing IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      _email, crypt('madagascat123@', gen_salt('bf')),
  ...
  ELSE
    _uid := _existing;
    UPDATE auth.users SET encrypted_password = crypt('madagascat123@', gen_salt('bf')),
      email_confirmed_at = COALESCE(email_confirmed_at, now()) WHERE id = _uid;
  END IF;
