...
-- helper to get/create a conversation between two users (canonical order)
...
-- Storage policies (one per bucket): public read, authenticated user can manage own folder (folder = auth.uid())
...
CREATE POLICY "Public read statuses" ON storage.objects FOR SELECT USING (bucket_id = 'statuses');
